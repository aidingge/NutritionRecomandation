...
class MenuViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_menus_by_elements(self, request):
        ...
        post = request.POST
        menus = None
        sql = 'SELECT mainApp_menu.name, mainapp_menu.calorie, mainapp_menu.minutes, mainapp_menu.flavor, ' \
              'mainapp_menu.technology, mainapp_menu.image_url, mainapp_menu.practice, mainapp_menu.elements_id ' \
              'FROM mainapp_menu INNER JOIN mainapp_element ON (mainapp_menu.elements_id = mainapp_element.id) where '
        query_sql = ''
        ...
        # query_sql = sql + '( ' + query_sql[5:] + ' )' 我改成下面那句了
...
class CookQuantityViewSet(viewsets.ReadOnlyModelViewSet):
    # 以下两行是我加的
    queryset = CookQuantity.objects.all()
    serializer_class = CookQuantitySerializer
