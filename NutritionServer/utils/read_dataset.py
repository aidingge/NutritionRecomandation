...
# done
def parse_physique():
   # physique_list = ['瘀血质', '阴虚质', '阳虚质', '痰湿质', '湿热质', '气郁质', '气虚质', '平和质']
    physique_list = ['阴虚质', '阳虚质', '痰湿质', '湿热质', '气郁质', '气虚质', '平和质']
    for each in physique_list:
        Physique.objects.create(physical_name=each)
...
def parse_physique_material():
    ...
    for i in range(len(table)):
        ...
        if isinstance(material_str, str):
...
def get_classification_list(name):
    """
    ...
    name_list = name_list[1:]  # 去掉第一个空的
    return name_list


def get_material_and_quantity_dict(name):
    """
...
def create_occupation():
    ...
    :return:
    """
    # job_list = ['农牧业','渔业','矿业','交通运输业','餐旅业','建筑工程业','制造业','新闻、出版、广告业','卫生','教师','金融业','IT业','体育']
    job_list = ['教师', '记者', '演员', '厨师', '医生', '护士', '司机', '军人', '律师', '商人', '会计', '程序员', '服务员', '作家', '模特', '导游', '歌手',
                '裁缝',
                '翻译', '法官', '保安', '花匠', '清洁工', '理发师', '消防员', '推销员', '运动员', '快递员', '外卖员', '主持人', '漫画家', '面点师', '美甲师',
                '保姆',
                '电力工程师', '化验员', '机长', '空姐', '画师', '动漫设计', '白领', '前台', '学生']
    for each in job_list:
        Occupation.objects.create(occupation_name=each)
